//
//  RasterizationImplementation.cpp
//  software-renderer
//
...
RasterizationImplementation::RasterizationImplementation()
{
...
float RasterizationImplementation::min3(const float &a, const float &b, const float &c)
{ return std::min(a, std::min(b, c)); }

float RasterizationImplementation::max3(const float &a, const float &b, const float &c)
{ return std::max(a, std::max(b, c)); }

float RasterizationImplementation::edgeFunction(const Vec3f &a, const Vec3f &b, const Vec3f &c)
...
//[/comment]
void RasterizationImplementation::computeScreenCoordinates(const float &filmApertureWidth,
...
//[/comment]
void RasterizationImplementation::convertToRaster(
    const Vec3f &vertexWorld, const Matrix44f &worldToCamera, const float &l,
    const float &r, const float &t, const float &b, const float &near,
    const uint32_t &imageWidth, const uint32_t &imageHeight, Vec3f &vertexRaster)
{
    Vec3f vertexCamera;
...
std::list<glm::vec2> RasterizationImplementation::startRaster()
{
    Cow cow;
    std::list<glm::vec2> pixels;
    ...
    for (uint32_t i = 0; i < ntris; ++i) {
        const Vec3f &v0 = cow.vertices[cow.nvertices[i * 3]];
        const Vec3f &v1 = cow.vertices[cow.nvertices[i * 3 + 1]];
        const Vec3f &v2 = cow.vertices[cow.nvertices[i * 3 + 2]];
        ...
        // [/comment]
        Vec2f st0 = cow.st[cow.stindices[i * 3]];
        Vec2f st1 = cow.st[cow.stindices[i * 3 + 1]];
        Vec2f st2 = cow.st[cow.stindices[i * 3 + 2]];
